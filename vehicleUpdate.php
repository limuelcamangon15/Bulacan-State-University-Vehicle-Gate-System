 <?php
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      if ($platePhotoPath) {
          $sql = "UPDATE vehicles 
                  SET VehicleType = ?, VehicleBrand = ?, VehicleModel = ?, OwnerID = ?, PlatePhoto = ? 
                  WHERE PlateNumber = ?";
          $stmt = $conn->prepare($sql);
          $stmt->bind_param("ssssss", $vehicleType, $vehicleBrand, $vehicleModel, $ownerID, $platePhotoPath, $plateNumber);
      } 
      else {     
          $sql = "UPDATE vehicles 
                  SET VehicleType = ?, VehicleBrand = ?, VehicleModel = ?, OwnerID = ? 
                  WHERE PlateNumber = ?";
          $stmt = $conn->prepare($sql);
          $stmt->bind_param("sssis", $vehicleType, $vehicleBrand, $vehicleModel, $ownerID, $plateNumber);
      }
...
<body>
  ...
  <div class="navbar" id="navbar">
        ...
        <form method="POST" enctype="multipart/form-data">
          <div class="row">
            <div class="input-group">
                ...
                <input type="text" id="plateNumber" name="plate_number" required readonly value="">
    ...
    <?php if (!empty($vehicleUpdated) && $vehicleUpdated): ?>
      <script>
        Swal.fire({
    ...
    <script>
      ...
      document.getElementById('plateNumber').value = getUrlParameter('plate_number');
      document.getElementById('vehicleType').value = getUrlParameter('vehicle_type');
      document.getElementById('vehicleBrand').value = getUrlParameter('vehicle_brand');
      document.getElementById('vehicleModel').value = getUrlParameter('vehicle_model');
      document.getElementById('ownerID').value = getUrlParameter('owner_id');
      document.getElementById('platePhoto').value = getUrlParameter('plate_photo');
        ...
        xhr.onload = function () {
          if (xhr.status === 200) {
            if (xhr.responseText === "exists") {
              isOwnerIdValid = true;
              document.getElementById('ownerID-error').style.display = "none";
            } else {
              isOwnerIdValid = false;
              document.getElementById('ownerID-error').style.display = "block";
            }
            validateForm();
          }
        };
      ...
      function validateForm() {
        submitButton.disabled = !isOwnerIdValid;
      }
    </script>
