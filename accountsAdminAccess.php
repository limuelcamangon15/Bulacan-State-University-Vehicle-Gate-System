 <?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['account_id'])) {
        ...
        $checkSql = "SELECT COUNT(*) FROM vehicles WHERE OwnerID = ?";
...
<body>
  ...
  <div class="navbar" id="navbar">
    ...
    <ul>
        ...
        <li><a href="accountsAdminAccess.php" class="active-nav"><i class="fas fa-users-cog"></i> Accounts</a></li>
  ...
  <div class="main-container">
    ...
    <div class="filter-section">
        <button class="add-btn" onclick="window.location.href='accountAdd.php'"><i class="fas fa-plus"></i> Add Account</button>

        <form method="GET" action="accountsAdminAccess.php" style="display:inline;">
                    ...
                    <?php while($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td class="action-btns">
                                ...
                                <button class="delete-btn" onclick="confirmDelete(<?php echo $row['AccountID']; ?>, '<?php echo $row['FirstName'];?>', '<?php echo $row['LastName'];?>')"><i class="fas fa-trash"></i> Delete</button>
                                <button class="update-btn" onclick="window.location.href='accountUpdate.php?account_id=<?php echo $row['AccountID']; ?>&first_name=<?php echo $row['FirstName']; ?>&last_name=<?php echo $row['LastName']; ?>&email=<?php echo $row['EmailAddress']; ?>&contact=<?php echo $row['ContactNumber']; ?>&username=<?php echo $row['Username']; ?>&password=<?php echo $row['Password']; ?>&role=<?php echo $row['Role']; ?>'"><i class="fas fa-pen-to-square"></i> Update</button>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                <?php else: ?>
    ...
    <script>
        function confirmDelete(id, fname, lname) {
            Swal.fire({
                title: 'Delete account of ' + fname + ' ' + lname + '?',
                html: 'Are you sure you want to delete the account of <strong>' + fname + ' ' + lname +'</strong>?',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    window.location.href = 'accountsAdminAccess.php?account_id=' +id;
                }
            });
        }
    </script>

    <?php if (!empty($accountDeleted) && $accountDeleted): ?>
    <script>
        Swal.fire({
            ...
            if(result.isConfirmed || result.dismiss){
                window.location.href = 'accountsAdminAccess.php';
            }
        });
    </script>
    <?php endif; ?>
