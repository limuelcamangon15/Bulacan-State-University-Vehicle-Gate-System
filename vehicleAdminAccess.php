 <?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        if (isset($_GET['plate_num']) && isset($_GET['action'])) {
            ...
            if ($newStatus !== null) {
                $stmt = $conn->prepare("UPDATE vehicles SET Status = ? WHERE PlateNumber = ?");
                $stmt->bind_param("ss", $newStatus, $plate);
                $stmt->execute();
                $stmt->close();
            }
        }
        ...
        if (isset($_GET['delete_plate_num'])) {
            $plate = $_GET['delete_plate_num'];
            $stmt = $conn->prepare("DELETE FROM vehicles WHERE PlateNumber = ?");
  ...
  <div class="main-container">
    ...
    <div class="summary-container">
        <div class="summary-card">
            <h3><i class="fa-solid fa-car-side" style="color:rgb(13, 127, 40);"></i> Recently Added Vehicle</h3>
            <?php 
                $resultRecentlyAdded = $conn->query("
                    SELECT CONCAT(vehicles.VehicleBrand, ' ', vehicles.VehicleModel) AS RecentlyAdded 
                    FROM vehicles 
                    ORDER BY PostedAt DESC 
                    LIMIT 1
                ");
                $row = $resultRecentlyAdded->fetch_assoc();
                $recentlyAddedVehicle = $row['RecentlyAdded'];
                echo "<p><strong><span id='newlyAdded'>$recentlyAddedVehicle</span></strong></p>";
            ?>
        </div>

        <div class="summary-card">
            <h3><i class="fas fa-car" style="color:rgb(156, 109, 44);"></i> Vehicles</h3>
            <?php 
                $resultTotalVehicles = $conn->query("SELECT COUNT(*) AS TotalVehicles FROM vehicles");
        ...
        <div class="summary-card">
            <h3><i class="fas fa-check-circle" style="color: #198754;"></i> Approved</h3>
            <?php 
        ...
        <div class="summary-card">
            <h3><i class="fas fa-ban" style="color: #dc3545;"></i> Rejected</h3>
            <?php 
                $resultTotalRejected = $conn->query("SELECT COUNT(*) AS TotalRejected FROM vehicles WHERE Status = 'Rejected'");
                $row = $resultTotalRejected->fetch_assoc();
                $totalRejected = $row['TotalRejected'];
                echo "<p><strong><span id='newlyAdded'>$totalRejected</span></strong></p>";
            ?>
        </div>
        ...
        <div class="summary-card">
            ...
            <?php 
                $resultTotalPending = $conn->query("SELECT COUNT(*) AS TotalPending FROM vehicles WHERE Status = 'Pending'");
                $row = $resultTotalPending->fetch_assoc();
                $totalPending = $row['TotalPending'];
                echo "<p><strong><span id='newlyAdded'>$totalPending</span></strong></p>";
            ?>
        </div>
    </div>

    <div class="filter-section">
        <button class="add-btn" onclick="window.location.href='vehicleAdd.php'"><i class="fas fa-plus"></i> Add Vehicle</button>

        <form method="GET" action="vehicleAdminAccess.php" style="display:inline;">
                    ...
                    <tr>
                        ...
                        <td class="action-btns">
                            ...
                            <?php if($row['Status'] === 'Pending'): ?>     
                                <button class="approve-btn" onclick="handleAction('<?php echo $row['PlateNumber']; ?>', 'approve')"><i class="fas fa-check"></i> Approve</button>
                                <button class="reject-btn" onclick="handleAction('<?php echo $row['PlateNumber']; ?>', 'reject')"><i class="fas fa-times"></i> Reject</button>
                            <?php endif; ?>
    ...
    <script>
        function handleAction(plateNumber, action) {
            let actionText = (action === 'approve') ? 'Approve' : (action === 'reject') ? 'Reject' : 'Proceed';

            Swal.fire({
                title: actionText + ' vehicle with plate number ' + plateNumber + '?',
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    window.location.href = 'vehicleAdminAccess.php?plate_num=' + plateNumber + '&action=' + action;
                }
            });
        }

        function confirmDelete(plateNumber, brand, model, ownername) {
            Swal.fire({
                title: 'Delete vehicle ' + brand + ' ' + model + '?',
                html: 'Are you sure you want to delete the vehicle owned by <strong>' + ownername + '</strong> with plate number <strong>' + plateNumber + '</strong>?',
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    window.location.href = 'vehicleAdminAccess.php?delete_plate_num=' + plateNumber;
                }
            });
        }
