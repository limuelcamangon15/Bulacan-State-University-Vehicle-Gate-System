 <?php
    ...
    if ($selectedCampus === 'All') {
        ...
        $stmt = $conn->prepare("SELECT * FROM campuses WHERE CampusName = ?");
        $stmt->bind_param("s", $selectedCampus);
        $stmt->execute();
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['campus_id'])) {
        $campusID = (int)($_GET['campus_id']);
        
        $sql = "DELETE FROM campuses WHERE CampusID = ?";
        ...
        if ($stmt->execute()) {
            ...
            echo "Error deleting campus: " . $stmt->error;
        }
...
<body>
  ...
  <div class="navbar" id="navbar">
    ...
    <ul>
        ...
        <li><a href="campusesAdminAccess.php" class="active-nav"><i class="fas fa-university"></i> Campuses</a></li>
  ...
  <div class="main-container">
    <div class="heading-div">
        <h1><i class="fa-solid fa-building-columns"></i> Manage Campuses</h1>
    </div>

    <div class="summary-container">
        <div class="summary-card">
            <h3><i class="fa-solid fa-school" style="color: #0d6efd;"></i> Recently Added Campus</h3>
            <?php 
                $resultRecentlyAdded = $conn->query("
                    SELECT CampusName AS RecentlyAdded 
                    FROM campuses 
                    ORDER BY CampusID DESC 
                    LIMIT 1
                ");
                $row = $resultRecentlyAdded->fetch_assoc();
                $recentlyAddedCampus = $row['RecentlyAdded'];
                echo "<p><strong><span id='newlyAdded'>$recentlyAddedCampus</span></strong></p>";
            ?>
        </div>

        <div class="summary-card">
            <h3><i class="fa-solid fa-building-columns" style="color: #6c757d;"></i> Campuses</h3>
            <?php 
    ...
    <div class="filter-section">
        <button class="add-btn" onclick="window.location.href='campusAdd.php'"><i class="fas fa-plus"></i> Add Campus</button>

        <form method="GET" action="campusesAdminAccess.php" style="display:inline;">         
    ...
    <table>
                    ...
                    <tr>
                        ...
                        <td class="action-btns">
                            <button 
                                    ...
                                    data-numofgates = "<?= $row['NumOfGates'] ?>"
                                >
                                    <i class="fas fa-eye"></i> View
                            </button>
                            
                            <button class="delete-btn" onclick="confirmDelete('<?php echo $row['CampusID']; ?>', '<?php echo $row['CampusName']; ?>')"><i class="fas fa-trash"></i> Delete</button>
                            <button class="update-btn" onclick="window.location.href='campusUpdate.php?campus_id=<?php echo $row['CampusID']; ?>&campus_name=<?php echo $row['CampusName'];?>&address=<?php echo $row['Address'];?>'"><i class="fas fa-pen-to-square"></i> Update</button>
                        </td>
                    </tr>
                <?php endwhile; ?>
            <?php else: ?>
                <tr><td colspan="9">No Campuses found.</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
    </div>
        ...
        <div id="modal" class="modal-overlay">
            <div class="modal-dialog">
                ...
                <div class="modal-header"><i class="fas fa-circle-info" style="margin-right: 8px;"></i> Campus Information</div>
                <div class="modal-body">             
                    <table class="modal-table">
                        <tbody>
                            <tr>
                                <td><i class="fas fa-id-badge" style="margin-right: 5px;"></i> Campus ID:</td>
                                <td><span id="modalCampusID"></span></td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                <td><i class="fas fa-map-marker-alt" style="margin-right: 5px;"></i> Address:</td>
                                <td><span id="modalAddress"></span></td>
                            </tr>
                            <tr>
                                <td><i class="fas fa-door-open" style="margin-right: 5px;"></i> Number of Gates:</td>
                                <td><span id="modalNumOfGates"></span></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    <script>
        function confirmDelete(id, campusName) {
            Swal.fire({
                title: 'Delete ' + campusName + '?',
                html: 'Are you sure you want to delete the <strong>' + campusName +'</strong>?',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    window.location.href = 'campusesAdminAccess.php?campus_id=' + id;
                }
            });
        }
    </script>

    <?php if (!empty($campusDeleted) && $campusDeleted): ?>
        <script>
            Swal.fire({
                title: 'Campus Deleted Successfully!',
                text: 'The campus has been deleted Successfully!',
                icon: 'success',
                ...
                if(result.isConfirmed || result.dismiss){
                    window.location.href = 'campusesAdminAccess.php';
                }
            });
        </script>
    <?php endif; ?>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.querySelectorAll('.view-btn').forEach(btn => {
                btn.addEventListener('click', () => {
                    ...
                    document.getElementById('modalCampusID').textContent = btn.dataset.campusid;
                    ...
                    document.getElementById('modalNumOfGates').textContent = btn.dataset.numofgates;
