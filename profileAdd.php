 <?php
    ...
    $stmt = $conn->prepare("INSERT INTO profiles (AccountID, Birthday, Gender, Street, Brgy, Town, Province, ProfilePhoto) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("isssssss", $accountID, $birthday, $gender, $street, $brgy, $town, $province, $profilePhotoPath );
...
<body>
  ...
  <div class="navbar" id="navbar">
    ...
    <div class="form-container">
        <h2>Add Profile</h2>
        <p>Fill up all fields to add a profile</p>

        <form method="POST" enctype="multipart/form-data">
            <div class="row">
            <div class="input-group">
                <label for="accountID">Account ID</label>
                <input type="number" id="accountID" name="account_id" required>
                <span id="account-error" class="error-message"><i class="fas fa-exclamation-triangle" style="color: #facc15; margin-right: 5px;"></i> A profile already exist for this account</span>
            </div>
    ...
    <?php if(!empty($profileAdded) && $profileAdded): ?>
    <script>
      Swal.fire({
        title: 'Profile Addded Successfully!',
  ...
  <script>
    ...
    const submitButton = document.querySelector('button[type="submit"]');
      ...
      accountIDInput.addEventListener('blur', checkProfile);

      function checkProfile() {
        const accountID = accountIDInput.value.trim();

        const xhr = new XMLHttpRequest();
        xhr.open('POST', 'checkProfile.php', true);
        ...
        xhr.onload = function () {
          if (xhr.status === 200) {
            ...
            if (response === "exists") {
              document.getElementById('account-error').innerHTML = '<i class="fas fa-exclamation-triangle" style="color: #facc15; margin-right: 5px;"></i> A profile already exists for this account.';
              document.getElementById('account-error').style.display = "block";
              isAccountValid = false;
            }
            else if (response === "invalid") {
              document.getElementById('account-error').innerHTML = '<i class="fas fa-exclamation-triangle" style="color: #facc15; margin-right: 5px;"></i> Account ID does not exist.';
              document.getElementById('account-error').style.display = "block";
              isAccountValid = false;
            } 
            else {
            ...
            submitButton.disabled = !isAccountValid;
          }
        };

        xhr.send('account_id=' + encodeURIComponent(accountID));
      }
  </script>
