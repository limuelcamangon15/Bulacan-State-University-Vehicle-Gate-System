 <?php
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        $stmt = $conn->prepare("INSERT INTO accounts (FirstName, LastName, EmailAddress, ContactNumber, Username, Password, Role) VALUES (?, ?, ?, ?, ?, ?, ?)");

        if ($stmt) {
            $stmt->bind_param("sssssss", $firstName, $lastName, $emailAddress, $contactNumber, $username, $hashedPassword, $role);

            if ($stmt->execute()) {
                $accountAdded = true;
            }
            else {
                echo "Error inserting account: " . $stmt->error;
            }
...
<body>
  ...
  <div class="form-container">
    <h2>Add Account</h2>
    <p>Fill up all fields to add an account</p>
  ...
  <?php if (!empty($accountAdded) && $accountAdded): ?>
    <script>
        Swal.fire({
