 <?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!empty($newPassword)) {    
            ...
            $sql = "UPDATE accounts 
                    SET FirstName = ?, LastName = ?, ContactNumber = ?, Role = ?, EmailAddress = ?, Username = ?, Password = ?
                    WHERE AccountID = ?";

            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssssssi", $firstName, $lastName, $contact, $role, $email, $username, $hashedPassword, $accountID);
        } 
        else {
            $sql = "UPDATE accounts 
                    SET FirstName = ?, LastName = ?, ContactNumber = ?, Role = ?, EmailAddress = ?, Username = ?
                    WHERE AccountID = ?";

            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssssi", $firstName, $lastName, $contact, $role, $email, $username, $accountID);
        }
...
<body>
  ...
  <div class="form-container">
    ...
    <form method="POST">
      <div class="input-group">
        <input type="hidden" id="accountID" name="account_id" required readonly>
      </div>
      ...
      <div class="input-group full-width">
        ...
        <div class="input-group full-width">
          ...
          <input type="password" id="password" name="password" placeholder="Keep this field empty to retain old password">
        </div>

      <button type="submit">Update Account</button>
    </form>
    <div class="input-group full-width">
      <button class="cancel-btn" type="none" onclick="window.location.href='accountsAdminAccess.php'">Cancel</button>
    </div>
  ...
  <script>
    ...
    document.getElementById('accountID').value = getUrlParameter('account_id');
    document.getElementById('firstName').value = getUrlParameter('first_name');
    document.getElementById('lastName').value = getUrlParameter('last_name');
    document.getElementById('contact').value = getUrlParameter('contact');
