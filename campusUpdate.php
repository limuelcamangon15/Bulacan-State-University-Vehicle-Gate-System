 <?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $sql = "UPDATE campuses 
                SET CampusName = ?, Address = ?
                WHERE CampusID = ?";
        
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssi", $name, $address, $campusID);
...
<body>
  ...
  <div class="navbar" id="navbar">
    ...
    <div class="form-container">
        ...
        <form method="POST">
            ...
            <div class="input-group">
                ...
                <input type="text" id="campusName" name="campus_name" required>
                <span id="campusName-error" style="height: 0px; align-self: center; color: red; font-weight: bold; margin-top: 3px; font-size: 13px; display: none;">Campus already exists</span>
            </div>
        ...
        <div class="input-group full-width">
          <button class="cancel-btn" type="none" onclick="window.location.href='campusesAdminAccess.php'">Cancel</button>
        </div>
    </div>
    ...
    <?php if (!empty($campusUpdated) && $campusUpdated): ?>
      <script>
          Swal.fire({
              title: 'Campus Updated Successfully!',
              text: 'The campus has been updated Successfully!',
              icon: 'success',
    ...
    <script>
      listenToScreenSize();
        ...
        document.getElementById('address').value = getUrlParameter('address');

      const campusNameInput = document.getElementById('campusName');
      const submitButton = document.querySelector('button[type="submit"]');
    
      let isCampusNameValid = false;
    
      campusNameInput.addEventListener('blur', checkCampusName);
      ...
      function checkCampusName() {
        ...
        xhr.open('POST', 'checkCampus.php', true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    
        xhr.onload = function () {
          if (xhr.status === 200) {
            if (xhr.responseText === "exists") {
              document.getElementById('campusName-error').style.display = "block";
              isCampusNameValid = false;
            }
            else {
              document.getElementById('campusName-error').style.display = "none";
              isCampusNameValid = true;
            }
            validateForm();
          }
        };
    
        xhr.send('campus_name=' + encodeURIComponent(name) + '&campus_id=' + encodeURIComponent(campusID));
      }
      
    
      function validateForm() {
        submitButton.disabled = !isCampusNameValid;
      }
    </script>
