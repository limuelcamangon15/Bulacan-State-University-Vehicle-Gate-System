 <?php
    ...
    $sql = "SELECT FirstName, LastName, ContactNumber, EmailAddress, Username FROM accounts WHERE AccountID = ?";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!empty($newPassword)) {
            ...
            $sql = "UPDATE accounts 
                    SET FirstName = ?, LastName = ?, ContactNumber = ?, EmailAddress = ?, Username = ?, Password = ?
                    WHERE AccountID = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssssi", $firstName, $lastName, $contact, $email, $username, $hashedPassword, $accountID);
        } 
        else {
            $sql = "UPDATE accounts 
                    SET FirstName = ?, LastName = ?, ContactNumber = ?, EmailAddress = ?, Username = ?
                    WHERE AccountID = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssssi", $firstName, $lastName, $contact, $email, $username, $accountID);
        }
...
<body>
  ...
  <div class="navbar" id="navbar">
    ...
    <ul>
        ...
        <div class="dropdown">
        <a href="accountGuard.php" class="active-nav"><i class="fas fa-user"></i> Account</a>
      ...
      <script>
          Swal.fire({
              ...
              if(result.isConfirmed || result.dismiss){
                  window.location.href = 'accountGuard.php';
              }
          });
      </script>
    <?php endif; ?>
